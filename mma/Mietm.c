...
void MieMma P(( double _tp1, double _tp2, double _tp3, double * _tp4, long _tpl4));
...
static const char* evalstrs[] = {
	"Mie[x_, mreal_, mimag_, mu_] := MieMma[N[x], N[mreal], N[mimag],",
	" Flatten[{N[mu]}]]",
	(const char*)0,
	"Mie::badDiameter = \"Size parameter x=pi*d/lambda must greater th",
	"an zero but less than 10000.\"",
	(const char*)0,
	"Mie::badRealIndex = \"Index of refraction must be positive and le",
	"ss than 10.\"",
	(const char*)0,
	"Mie::badImagIndex = \"Imaginary part of index of refraction must ",
	"be non-positive.\"",
	(const char*)0,
	"Mie::badAngle = \"List of angles must be a list of cosines of the",
	" angles.\"",
	(const char*)0,
	(const char*)0
};
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"MieMma[x_Real, mreal_Real, mimag_Real, mu_List]", (char *)"{ x, mreal, mimag, mu}", 0);
